...
Note that if we had more than 1 document, we would get a count of how many documents in which the tokens appear, and we can assign documents to grouping variable. This will become useful later.
...
As the above code block suggest, the `tokens()` function in [**quanteda**](http://quanteda.io/reference/tokens.html) gives you some measure of control.
...
An alternative to making tokenizing decisions inside the tokenizing process, you can process the text before tokenizing using functions for manipulating strings in **stringr**, **stringi**, **textclean**, or base R (like `grep( )`). Some common and convenient transformations are wrapped in a function called `preprocess_text( )` provided in `quanteda.extras`:
...
corpus_comp <- data.frame(frequency = ntoken(comb_dfm)) |>

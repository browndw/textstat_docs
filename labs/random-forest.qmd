...
We're going to repeat the Federalist classification problem. Rather than a lasso regression model, however, this time we'll be constructing a random forest classification model.
...
And we'll start by setting up our data much like we did for lasso. First, we'll
get the metadata, which is provided in the `cmu.textstat` package:
...
# Note that some R functions have difficulty when column names are same as base
# function names like 'in' or 'if'. To head off any problems, we'll convert the
# first letter to upper case.
colnames(train_dfm)[-1] <- colnames(train_dfm)[-1] |>
  str_to_title()
...
colnames(test_dfm)[-1] <- colnames(test_dfm)[-1] |>
  str_to_title()
```

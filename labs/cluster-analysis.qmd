...
Our clustering will be based on part-of-speech counts, so we need to parse some data using **udpipe**. First, we'll load the `micusp_mini` corpus from the cmu.textstat packageand subset out the Biology and English papers.
...
**Note!** Look at lab 7, `feature-engineering.qmd`, to see how to download the udpipe model. You already downloaded one for that lab, so change the path in `udpipe_load_model()` to point to where it was saved, or download the model again.
...
fviz_dist(d, gradient = list(low = "steelblue", mid = "white", high = "tomato"))
```
...
Dimension reduction for plotting k-means is typically done using PCA. So let's start there.

```{r}
km_pca <- prcomp(sub_dfm)
```

We can check the percent of variance explained by looking at the eigenvalues.

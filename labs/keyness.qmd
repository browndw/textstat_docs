...
Expected frequencies are based on the relative size of each corpus (in total number of words C~i~) and observed frequencies:
...
$$
LL = 2 \times \sum_i \sum_j O_{ij} \log \frac{O_{ij}}{E_{ij}}
$$
...
We'll do the same comparison for fiction against academic writing.
...
To produce a data.frame comparing more than two sub-corpora, use the `keyness_pairs()` function:

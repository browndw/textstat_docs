...
For parallel processing in R, we'll use the package **future.apply**.
...
```{r anno_splits}
annotation <- future_lapply(corpus_split, annotate_splits, future.seed = TRUE)
```

As you might guess, the output is a list of data frames, so we'll combine them using **bind_rows()**.
...
Next, we can use the function **keywords_phrases()** to extract phrase-types based on regular expressions. Refer to the documentation for suggested **regex** patterns: <https://cran.r-project.org/web/packages/udpipe/refman/udpipe.html#keywords_phrases>.

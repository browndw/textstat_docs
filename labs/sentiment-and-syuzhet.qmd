...
```{r}
# str_squish() is a useful function from readr for getting rid of extra spaces,
...
```{r}
#| fig-cap: "Sentiment in *Madame Bovary* using transformed values."

plot(mb_dct, type = "l", xlab = "Narrative Time",
...
```{r}
mb_df <- data.frame(sentiment = mb_sentiment) |>
  rownames_to_column("time") |>
  mutate(time = as.numeric(time)) |>
...
ggplot(data = mb_dct, aes(x = time, y = dct)) +
  ...
  geom_point(data = mb_df, aes(x = time, y = sentiment),
             alpha = 0.25, size = 0.25) +
  xlab("Normalized Narrative Time") +

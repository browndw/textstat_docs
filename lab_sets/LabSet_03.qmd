...
::: {.callout-note}
The preview of this lab set is rendered in HTML. However, all assignments must be rendered in PDF for submission on Canvas. The **textstat_tools** repository is already set up to do this for you. Be sure to [follow the directions](https://quarto.org/docs/output-formats/pdf-basics.html) including the installation of **tinytex**.
:::
...
Following the example in Brezina (pg. 129), report and briefly interpret the output of the regression model you made to evaluate usage of `which` and `that` (**glm_fit**).

> Your response

## Task 2

Write a brief interpretation of the probability curves you created (in the chunk named `probability-curves` at the end of the lab):

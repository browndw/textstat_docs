...
::: {style="font-size: 65%;"}
$$
\text{PMI} = \log_{2} \left( \frac{O_{11}}{E_{11}} \right)
$$
...
* $R_1$ is the frequency of the node in the whole corpus
* $C_1$  is the frequency of the collocate in the whole corpus
* The accepted cutoff for a meaningful association is PMI > 3
...
$$
\begin{aligned}
\text{PMI}^k = \log_{2} \left( \frac{O_{11}{^k}}{E_{11}} \right) = \text{PMI} - (1 - k)\times\log_{2} \left( O_{11} \right)
\\
\text{where}~2 \le k \le 3
\end{aligned}
$$

$$
\text{NPMI} = \frac{\text{PMI}}{-\log_{2}(O_{11})}
$$
:::

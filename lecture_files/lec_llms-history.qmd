...
$$
\text{cosine similarity} = S_{c}(A, B) := \cos(\theta) = \frac{A \cdot B}{||A||~||B||}
...
-  Then, we tagged the data using Biber's [-@biber1991variation] features (which counts things like [passives, nominalizations, attributive adjectives](https://cran.r-project.org/web/packages/pseudobibeR/refman/pseudobibeR.html#biber), etc.).

## Investigating LLM-generated text

-   It turns out, machine-authored prose and human-authored prose don’t really look the same in their morphosyntactic and functional features. [@DeLuca:2024; @herbold2023large; @markey2024dense]
...
::: {style="font-size: 60%;"}
-   We also created an experiment at scale [@Reinhart:2024b], with 10,000 samples, across 9 text-types.
:::
